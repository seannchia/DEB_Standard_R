fnfourier<-function(t, p){
  ...
  n = nrow(p)     # number of fourier terms +1
  period = p[1,1] # period of periodic function
  a0 = p[1,2]     # mean level of function
  a = p[2:nrow(p),1]    # fourier coefficients for cosinus
  b = p[2:nrow(p),2]    # fourier coefficients for sinus
  n = n - 1       # number of Fourier terms
  
  T = (2 * pi * t/period - pi) * seq(from=1,to=n,by=1) # matrix of times, order
